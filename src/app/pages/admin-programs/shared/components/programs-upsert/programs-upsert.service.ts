...
import {ProgramsUpsertComponent} from './programs-upsert.component';
import {ProgramsApiService} from '../../services/programs.api.service';

@Injectable({
  providedIn: 'root'
})
export class ProgramsUpsertService {
  component: ProgramsUpsertComponent;
  ...
  save() {
    ...
    if (this.component.date) {
      const deadline = new Date(this.component.date);
      this.component.request.deadline = deadline.toISOString();
    }
    if (this.isValid()) {
